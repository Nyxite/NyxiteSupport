...
- **Operator UI** — this service ships **its own** operator helpdesk UI (queue, ticket detail, triage, reply, tags/assignee, search). `NyxiteAdmin` only **links out** to it, and only on instances where reporting is enabled (SUP-9); the UI itself is admin-only, reachable over the maintainer's admin network (WireGuard), consistent with `NyxiteAdmin`'s exposure tier.
